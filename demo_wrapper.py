...
extensions_dir={'noextension': False, 'ghostery': False, 'disconnect': False, 'ublock-origin': False, 'https-everywhere': False}
runs=1

# loop over category
for category, site_list in sites_dir.items():
	...
	for site in site_list:
		...
		for extension, flag in extensions_dir.items():

			# make extension folder
			extension_path=site_path+'/'+extension
			...
			for run in range(runs):
				...
				print('\n')
				print(category+' - '+site+' - '+extension+' - '+str(run))
				...
				helper_run(run_path, [site], ext_flags)
